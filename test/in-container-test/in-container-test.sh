...
docker build --no-cache -f tester-container.Dockerfile -t alfipbus-tester .
if [ $? -ne 0 ]; then
    print_failure "alfipbus-tester build"
fi

docker build --no-cache -f ../../Dockerfile -t alfipbus .
if [ $? -ne 0 ]; then
...
# Run mock
docker run -i -d --name tester-mock --rm --mount type=bind,source=./common-storage,target=/common-storage --network alfipbus-tester-network --add-host host.docker.internal:host-gateway --ip 172.25.75.10 alfipbus-tester
docker exec -d -e LD_LIBRARY_PATH="/usr/lib" tester-mock  /bin/bash -c "/home/alf-ipbus-tester/build/bin/mock -c /common-storage/test-configuration.toml -f /common-storage/output/mock-log -v"
...
docker exec -d -e LD_LIBRARY_PATH="/usr/lib" -e DIM_HOST_NODE=172.25.75.12 -e DIM_DNS_NODE=host.docker.internal tester-alf /bin/bash -c "AlfIPbus -n ALF -l 172.25.75.10:50001 -f /common-storage/output/alf-log -t 1000 -v"
