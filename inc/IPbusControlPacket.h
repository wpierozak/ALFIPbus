...
*/
    void addWordToWrite(uint32_t address, uint32_t value) { addTransaction(data_write, address, &value, 1); }
    ...
    void addNBitsToChange(uint32_t address, uint32_t data, uint8_t nbits, uint8_t shift = 0) {
        if (nbits == 32) { addTransaction(data_write, address, &data, 1); return; }
