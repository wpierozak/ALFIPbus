      ...
      switch (th->typeID) {
        ...
        case ConfigurationRead: {
          ...
          if (th->words > wordsAhead) {
            ...
            if (th->infoCode == 0) {
              std::string message = "Read transaction from " + std::to_string(*m_transactionsList.at(i).address) + " - received " + std::to_string(wordsAhead) + " words, expected: " + std::to_string(th->words);
